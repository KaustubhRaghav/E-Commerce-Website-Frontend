...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      setAddresses((prevState) => ({ ...prevState, all: successResult.data }));
  ...
  const validateRequest = (items, addresses) => {
    ...
    let totalCartBalance = items.reduce(
      (total, current) => (total += current.cost * current.qty),
      0
    );

    if (balance < totalCartBalance) {
      enqueueSnackbar(
        "You do not have enough balance in your wallet for this purchase",
        { variant: "warning" }
      );
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    if (isValid) {
      try {
        ...
        localStorage.setItem("balance", balance - totalCartBalance);
        ...
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {addresses.all.length > 0 && (
                <Box display="flex" flexDirection="column">
                  {addresses.all.map((currentAddress) => (
                    <Box
                      key={currentAddress._id}
                      className={`address-item ${
                        addresses.selected === currentAddress._id
                          ? "selected"
                          : "not-selected"
                      }`}
                      onClick={() => handleAddressSelect(currentAddress._id)}
                    >
                      ...
                      <Button
                        ...
                        startIcon={<Delete />}
                        onClick={(event) =>
                          deleteAddress(event, token, currentAddress._id)
                        }
                      >
                        DELETE
                      </Button>
